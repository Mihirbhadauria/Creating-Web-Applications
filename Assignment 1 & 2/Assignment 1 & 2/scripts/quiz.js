...
function calculateScore(){
    ...
    if(first_search_engine == "archie"){
        scount.push("*");
    }

    if(javaScript + aliexpress  == "javascript" + "aliexpress"){
        scount.push("*");
    }

    if(define_search_engine.includes == "web based tool" || "search information"){
        scount.push("*");
    }

    if(percentage_of_usage_of_google == "92"){
        scount.push("*");
    }

    score = scount.length;
    if (score == 0){
        alert("You have to answer one question correctly before submitting, Please try again.\n")
    }
